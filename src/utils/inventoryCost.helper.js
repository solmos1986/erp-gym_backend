...
export async function calculateCost(tx, companyId, branchId, productId, quantity, unitCost) {
  ...
  switch (company.costMethod) {
    case "WEIGHTED_AVERAGE":
      return await calculateWeightedAverage(tx, branchId, productId, quantity, unitCost);
...
async function calculateWeightedAverage(tx, branchId, productId, quantity, unitCost) {
  ...
  const currentCost = Number(productBranch.unitCost);
 ...
 */
async function calculateFIFO(tx, branchId, productId, quantity, unitCost) {
  throw new Error("Método FIFO aún no implementado.");
}
 ...
 * (Pendiente de implementación)
 */
async function calculateStandardCost(tx, branchId, productId, quantity, unitCost) {
  throw new Error("Método STANDARD aún no implementado.");
}
